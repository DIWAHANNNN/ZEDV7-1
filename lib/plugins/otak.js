...
const info = $(d).find('div.thumb > a > div.thumbz > h2.jdlflm').text()
const info = $(d).find('div.thumb > a > div.thumbz > img').attr('src')
...
result.push({ info, thumb, eps, hri, tgl, link })
})
return result
}
...
hasil.push({ info_, link_, thumb_, genre, status, rating, producer, total_eps, sinopsis, japanese_title, tipe, durasi, tgl_rilis, studio, batch, link_dl,tigaDrive,empatDrive,tujuhDrive })
